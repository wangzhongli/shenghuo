...
public class PagerStrip extends HorizontalScrollView {
	...
	// @formatter:on
	private LinearLayout.LayoutParams defaultTabLayoutParams;// 默认
	private LinearLayout.LayoutParams expandedTabLayoutParams;// 扩展
	// 监听
	...
	private Paint rectPaint;// 矩形绘

	private Paint dividerPaint;// 分割绘

	private int indicatorColor = 0xffd80c18;// 指示颜色

	private int underlineColor = 0xffe0e0e0;// 下划线颜色

	private int dividerColor = 0xfff2f2f2;// 分割颜色
	...
	private int tabTextColor = 0xff767676;
	...
	public PagerStrip(Context context, AttributeSet attrs,
		...
		a = context.obtainStyledAttributes(attrs,
				R.styleable.PagerStrip);

		indicatorColor = a.getColor(
				R.styleable.PagerStrip_pstsIndicatorColor,
				indicatorColor);
		underlineColor = a.getColor(
				R.styleable.PagerStrip_pstsUnderlineColor,
				underlineColor);
		dividerColor = a
				.getColor(R.styleable.PagerStrip_pstsDividerColor,
						dividerColor);
		indicatorHeight = a.getDimensionPixelSize(
				R.styleable.PagerStrip_pstsIndicatorHeight,
				indicatorHeight);
		underlineHeight = a.getDimensionPixelSize(
				R.styleable.PagerStrip_pstsUnderlineHeight,
				underlineHeight);
		dividerPadding = a.getDimensionPixelSize(
				R.styleable.PagerStrip_pstsDividerPadding,
				dividerPadding);
		tabPadding = a.getDimensionPixelSize(
				R.styleable.PagerStrip_pstsTabPaddingLeftRight,
				tabPadding);
		tabBackgroundResId = a.getResourceId(
				R.styleable.PagerStrip_pstsTabBackground,
				tabBackgroundResId);
		shouldExpand = a
				.getBoolean(R.styleable.PagerStrip_pstsShouldExpand,
						shouldExpand);
		scrollOffset = a
				.getDimensionPixelSize(
						R.styleable.PagerStrip_pstsScrollOffset,
						scrollOffset);
		textAllCaps = a.getBoolean(
				R.styleable.PagerStrip_pstsTextAllCaps, textAllCaps);
...
//					tab.setTextColor(getResources().getColor(R.color.titleBar_background));
					tab.setTextColor(tabTextColor);
				}else{
					tab.setTextColor(tabTextColor);
				}
	...
	protected void onDraw(Canvas canvas) {
		...
		canvas.drawRect(0, height - indicatorHeight, tabsContainer.getWidth(),
				height, rectPaint);
		// draw indicator line
